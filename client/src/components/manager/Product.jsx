...
//                 console.log("formDataaaaa: ",formData._id);
//                 updateProd(formData)
//                 refetch()
...
//         return <img  src={`http://localhost:7777/uploads/${rowData?.image}`} alt={rowData.image} className="shadow-2 border-round" style={{ width: '64px' }} />;
//     };
...
import { InputText } from 'primereact/inputtext';
import { Tag } from 'primereact/tag';
import { useGetProdWithCategoryNameQuery, useGetAllCategoriesQuery, useAddNewProdMutation, useDeleteProdMutation, useUpdateProdMutation } from '../../features/manager/ManagerProductApiSlice';
...
export default function Product() {
    ...
    const { data: products = [], isSuccess: isSuccessProducts } = useGetProdWithCategoryNameQuery();
    const { data: allCategories = [], isSuccess: isSuccessCategories } = useGetAllCategoriesQuery();

    const [createProd] = useAddNewProdMutation();
    ...
    const saveProduct = async () => {
        ...
        if (product.name.trim()) {
            ...
            if (product._id) {
                await updateProd(formData);
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Product Updated', life: 3000 });
            } else {
                await createProd(formData);
                toast.current.show({ severity: 'success', summary: 'Successful', detail: 'Product Created', life: 3000 });
            }
    ...
    const onCategoryChange = (e) => {
        let _product = { ...product };
        _product['category'] = e.target.value;
        setProduct(_product);
    };
    ...
    return (
        <div dir="ltr">
            ...
            <Dialog visible={productDialog} style={{ width: '32rem' }} header="Product Details" modal className="p-fluid" footer={productDialogFooter} onHide={hideDialog}>
                <div className="field">
                ...
                <div className="field">
                    ...
                    <div className="formgrid grid">
                        {isSuccessCategories && allCategories.map((category) => (
                            <div key={category._id} className="field-radiobutton col-6">
                                <RadioButton inputId={category._id} name="category" value={category._id} onChange={onCategoryChange} checked={product.category === category._id} />
            ...
            <Dialog visible={deleteProductDialog} style={{ width: '32rem' }} header="Confirm" modal footer={deleteProductDialogFooter} onHide={hideDeleteProductDialog}>
                <div className="confirmation-content">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
